...
| **Documents (Word / PDF)** | install the `.ttf`, paste encoded text, export. Word / Pages / InDesign only, **never Google Docs** (it can't load custom fonts) | branded `shieldfont-*.ttf` | full `"ShieldFont Optik"`, or `"ShieldFont Optik Max"` for the maxhide build | Regular (400) only | ●○○ 1/3 |
