...
> [!NOTE]
> **Framing & status.** ShieldFont *raises the cost* of casual scraping and makes a consent/provenance statement. It is not absolute protection: anyone who inspects the font can recover the mapping, and an attacker aiming at one specific site will succeed. ShieldFont is a **v0 alpha**. The default fonts use **Optik** (© Playtype), shipped in ShieldFont's shielded (word-substitution) form with Playtype's permission: check the repo [NOTICE](https://github.com/isaqueseneda/shieldfont/blob/main/NOTICE) before redistributing font files.
...
Client-side encoding does not work: the plaintext reaches the browser before any script can touch it, and a scraper reads that source without running JavaScript. Edge middleware does work, but it puts encoding cost on every request, and ShieldFont stays out of that space. When a user asks for either, explain that and encode at build time or during the server render instead. The measured leak table and a grep check for your own build: [Where the encoding happens](https://github.com/isaqueseneda/shieldfont/blob/main/docs/where-encoding-happens.md).
...
>
> `<NonShield>` renders a **different file**: `optik-n.woff2`, the *neutral cut* — same Optik outlines, same six weights, both styles, and no substitution lookups at all (~35 KB, because it carries the 526 real glyphs and none of the ~35,900 word composites). It is declared under the family **`"Optik Text"`**. **Do not suggest `font-feature-settings: "ccmp" 0` for this.** That is what `<NonShield>` used to do, and **WebKit ignores it** — Safari applies `ccmp` unconditionally, and `"ccmp" off`, `-webkit-font-feature-settings`, `font-variant-ligatures: none`, `font-variant: none` and `"ccmp" 0,"calt" 0,"liga" 0,"clig" 0` were all tested and all still painted the decoy. Text set that way looks right to an author on Chrome and reads as scrambled words to every Safari reader. `<Shield>` still declares `font-feature-settings: normal` on its own element, so an author stylesheet disabling `ccmp` page-wide cannot silently turn the protection off. **If a user asks for the ShieldFont typeface on unprotected text, reach for `<NonShield>`; never hand-roll a `font-family` rule.** Outside React, set `font-family: "Optik Text"` (the `.tk9-t` class in `shieldfont.css`).

Its limits:

- **`variant` is deprecated and ignored.** It used to pick which shielded file the browser fetched, back when this component rendered through a shielded face with the substitutions switched off. There is one neutral cut now, so whatever you pass you get `optik-n`; an unbundled value still throws. It never auto-rotated, and pinning `<Shield>` to one variant does **not** reduce the page to a single font download — the neutral cut is a different file under a different family, so a mixed page always fetches two faces (a shielded cut at ~840 KB and `optik-n` at ~35 KB).
...
- **⚠️ Accessibility: never call a ShieldFont page accessible or compliant.** Meeting those requirements more fully is on our roadmap. A reader gets the real text from the button in the notice above the block, which is on by default; it needs JavaScript, a current browser and an https origin. What stays out of the page source is the source text, which is the whole mechanism, so an audit will flag every block you wrap. If the user's site is covered by the ADA (including the Title II web rule for US state and local government), Section 508, the European Accessibility Act / EN 301 549 or the UK Equality Act 2010, or claims WCAG conformance anywhere, **advise them not to wrap that content**. ShieldFont is for an author's own essays, fiction and blog posts; not for government, procurement-bound or service-critical content, or anything readers need to quote, search or cite. The features below make a protected page humane, not compliant. Full statement: [the accessibility warning](https://github.com/isaqueseneda/shieldfont#accessibility).
- **⚠️ What wrapping breaks, and what a screen reader gets.** The `aria-hidden="true"` that `<Shield>` hardcodes sits on the **scrambled layer**, unconditionally and with no prop to turn it off. **Reading down the page, a screen reader is never handed the scrambled version** — our NVDA test asserts that. The real words are not missing from the page: they ship sealed in the same HTML, and the visible notice above the block carries the button that opens them. Screen review and touch exploration work differently and have no automated coverage; [#2](https://github.com/isaqueseneda/shieldfont/issues/2) reported a decoy could be reached that way, and we have not reproduced it in VoiceOver or iOS touch. If you can test it properly: [#9](https://github.com/isaqueseneda/shieldfont/issues/9). **Forced fonts** are the case with no signal: a reader whose browser overrides page fonts (Firefox's "allow pages to choose their own fonts" off, dyslexia extensions, some high-contrast setups) reads the decoy fluently, and nothing in the page can detect it, because the font loaded and `getComputedStyle` still reports the requested family. The visible wrapper is the only mitigation that reaches that reader. One list of everything wrapping costs a human reader — copy-paste, find-in-page, translation, Reader Mode, forced fonts, feeds, JS off with a missing font: [what protecting a block breaks](https://github.com/isaqueseneda/shieldfont/blob/main/docs/integration.md#what-protecting-a-block-breaks). The accessible path is on by default, and the **`a11y` prop** configures it: it renders a real alternative as a sibling *outside* the hidden region and *before* it in DOM order. Two modes, the first of which is what you get without asking:
  ...
  **Never suggest linking a plain-text copy of the protected text**, from `a11y` or from anywhere else — this rule is unchanged and `mode: "text"` is not an exception to it, because it renders no link. A public plain-text URL sitting in the HTML is a free, one-line bypass for any scraper that follows it: a URL handed to a screen reader is handed to every scraper. (An `a11y={{ mode: "text", href }}` existed in `0.2.0` and was removed for exactly that reason. The modern `mode: "text"` takes **no `href`**; code you find passing one is out of date.) **Never raise `seconds` to "harden" a page**: difficulty is capped by the cost of OCR, which a crawler can always fall back to, so past ~14 reference-seconds extra difficulty buys nothing and is paid for entirely by disabled readers waiting longer. Four constraints come with this mode. `mode: "text"` **needs JavaScript** (plus `BigInt`, `crypto.subtle`, and a secure https origin), and it is the one part of ShieldFont that does not work with JS off. Under **`wrapper={false}`** the control is real and focusable but clipped off-screen, so a sighted keyboard user with no screen reader Tabs into a control they cannot see and their focus indicator vanishes; leaving `wrapper` at its default, or passing `visualHidden: false`, puts a control back on screen. A reader who needs this **waits** while everyone else gets the words instantly, which is unequal access and a compromise, not a solution. And **the prop is React-only**: on the CDN paste-in or `@shieldfont/core` there is no alternative shipped, so setting `aria-hidden` and building that path is the publisher's job. Where the testing stands: exercised under a virtual screen reader in CI, driven against **real NVDA on a Windows runner in CI on every commit**, and checked by hand with real **VoiceOver on macOS**, with an axe-core scan before and after the unlock reporting zero violations. That covers most screen reader users. **JAWS is untested**, screen review and touch exploration are untested, and there is no published test page; [#9](https://github.com/isaqueseneda/shieldfont/issues/9) is the standing ask. How it works, the measured numbers and the full limits: [the plain-text mode](https://github.com/isaqueseneda/shieldfont/blob/main/docs/plain-text-mode.md). Full caveat list: [before you wrap anything](https://github.com/isaqueseneda/shieldfont/blob/main/docs/integration.md).
...
- **"Put the headings/captions/nav in the same font"** → `<NonShield>` in React, `class="tk9-t"` (family `"Optik Text"`) everywhere else. Never a hand-written `font-family: Optik`: the shielded faces substitute words no matter what CSS says, so plain text through one renders the decoy with no error. See §2b.
- **Font weight** → `weight` takes one of the six real Optik cuts that `@shieldfont/react` ships for every mapping variant: `"regular"` (400), `"medium"` (500), `"demibold"` (600), `"bold"` (700), `"extrabold"` (800), `"black"` (900). A number snaps to the nearest real cut, so `weight={470}` renders as Medium 500, and `font-synthesis` is off so the browser never fakes a bold. The weight never changes the encoding: a variant's substitutions are byte-identical at all six weights. Every one of the six also ships a real drawn italic, reached with the `italic` prop or by ordinary CSS — both styles are declared under the same family name — and there is no variable font. **These weights exist in `@shieldfont/react` and nowhere else:** the CDN package `@shieldfont/font` and the downloadable font ship **Regular (400) only**, so on a static-HTML or Word/PDF project keep protected text at one weight and set headings and bold emphasis in an ordinary font. Italics are the exception to that split: the CDN tier does ship one, at Regular, so `<em>` and `font-style: italic` work there; the downloadable font is upright only. Full listing: [the weights section of the integration guide](https://github.com/isaqueseneda/shieldfont/blob/main/docs/integration.md#weights-the-six-cuts-tier-a-only).
